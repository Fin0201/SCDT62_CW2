...
export default function EditWorkoutActivity({ selectedWorkout, onWorkoutSuccess }) {
    ...
    <View key={key}> 
      <Card key={key} style={styles.item} containerStyle={styles.itemContainer}>
        ...
        <View style={{ justifyContent: 'space-evenly', flexDirection: 'row', marginTop: 20 }}>
          <Button title="Select Activity" color='green' onPress={() => { setSelectActivity(val); setShowEditWorkout(true); }} /> 
        </View>
      </Card>
    </View>
  ));
  ...
  return (    
    <View style={styles.container}>
      ...
      <Overlay isVisible={showEditWorkout} onBackdropPress={() => setShowEditWorkout(false)}>
        <EditWorkout selectedWorkout={selectedWorkout} selectedActivity={selectActivity} onWorkoutSuccess={() => { fetchActivities(); setShowEditWorkout(false); onWorkoutSuccess(); }} />
      </Overlay>
